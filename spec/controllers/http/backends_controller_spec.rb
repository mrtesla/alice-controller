...
describe Http::BackendsController do
  ...
  describe "GET index" do
    it "assigns all http_backends as @http_backends" do
      backend = Http::Backend.create! valid_attributes
      get :index
      assigns(:http_backends).should eq([backend])
    end
  end
  ...
  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested backend" do
        backend = Http::Backend.create! valid_attributes
        # Assuming there are no other http_backends in the database, this
        # specifies that the Http::Backend created on the previous line
    ...
    describe "with invalid params" do
      it "assigns the backend as @backend" do
        backend = Http::Backend.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Http::Backend.any_instance.stub(:save).and_return(false)
  ...
  describe "DELETE destroy" do
    it "destroys the requested backend" do
      backend = Http::Backend.create! valid_attributes
      expect {
        delete :destroy, :id => backend.id
      }.to change(Http::Backend, :count).by(-1)
    end

    it "redirects to the http_backends list" do
      backend = Http::Backend.create! valid_attributes
      delete :destroy, :id => backend.id
      response.should redirect_to(http_backends_url)
    end
  end
