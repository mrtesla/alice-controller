...
module Alice::Chef
  ...
  class ProcessUpdater
    ...
    def update
      ...
      new_processes = new_processes.index_by { |p| Digest::SHA1.hexdigest([application_key, ui_name].join('')) }

      query = C::Search::Query.new
      query.search('alice-processes', "application_key:#{application_key}").each do |process|
